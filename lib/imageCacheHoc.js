...
export default function imageCacheHoc(Image, options = {}) {
  ...
  if (options.defaultRejected && (!options.defaultRejected.component || !options.defaultRejected.props)) { throw new Error('defaultRejected option object must include "component" and "props" properties (props can be an empty object)'); }
  ...
  return class extends React.PureComponent {
     ...
     * @param headers {Object} - Object containing request headers.
     * @returns {Promise} promise that resolves to an object that contains cached file info.
     */
    static async cacheFile(url, permanent = false, fileName, headers) {

      const fileSystem = FileSystemFactory();
      const localFilePath = await fileSystem.getLocalFilePathFromUrl(url, permanent, fileName, headers);
    ...
    async componentDidMount() {
      ...
      const fileName = this.props.fileName || await this.fileSystem.getFileNameFromUrl(url, this.props.headers);
      FileSystem.lockCacheFile(fileName, this.componentId);

      // Init the image cache logic
      await this._loadImage(url, fileName);
    ...
    async componentWillReceiveProps(nextProps) {
      ...
      const fileName = this.props.fileName || await this.fileSystem.getFileNameFromUrl(url);
      const nextFileName = nextProps.fileName || await this.fileSystem.getFileNameFromUrl(nextUrl);

      FileSystem.unlockCacheFile(fileName, this.componentId);
      FileSystem.lockCacheFile(nextFileName, this.componentId);

      // Init the image cache logic
      await this._loadImage(nextUrl, nextFileName, nextProps.headers);
     ...
     * @param headers {Object} - Object containing remote request's headers.
     * @private
     */
    async _loadImage(url, {fileName = this.props.fileName, headers = this.props.headers, extension = this.props.extension} = {}) {
      // Check local fs for file, fallback to network and write file to disk if local file not found.
      const permanent = !!this.props.permanent;
      let localFilePath = null;
      
      try {
        localFilePath = await this.fileSystem.getLocalFilePathFromUrl(url, {permanent, extension, fileName, headers});
      } catch (error) {
        console.warn(error); // eslint-disable-line no-console
        this.props.onReject && this.props.onReject(error);
    ...
    render() {
      ...
      if (this.state.localFilePath) {
        ...
        if (this.props.rejected) {
          return (<this.props.rejected.component {...this.props.rejected.props} />);
        }
        if (this.options.defaultRejected) {
          return (<this.options.defaultRejected.component {...this.options.defaultRejected.props} />);
        }
