...
export class FileSystem {
   ...
   */
  async getFileNameFromUrl(url, {headers, fileName, extension} = {}) {
    ...
    if (!_extension) {
      switch (urlExt) {
        ...
        case 'bmp':
          ...
          break;
        default:
          _extension = await this.getExtensionFromContentTypeHeader(url, {headers});
      }
    }
   ...
   */
  async getLocalFilePathFromUrl(url, {permanent = false, fileName = null, extension = null, headers = {}} = {}) {

    let filePath = null;

    let _fileName = await this.getFileNameFromUrl(url, {headers, extension, fileName});
    ...
    if (exists[0]) {
      ...
      let result = await this.fetchFile(url, {permanent, fileName: _fileName, extension, headers, clobber: true}); // Clobber must be true to allow concurrent CacheableImage components with same source url (ie: bullet point images).
   ...
   */
  async fetchFile(url, {permanent = false, fileName = null, extension = null, headers = {}, clobber = false} = {}) {
    let _fileName = await this.getFileNameFromUrl(url, {headers, fileName, extension});
    
    let path = this.baseFilePath + (permanent ? 'permanent' : 'cache') + '/' + _fileName;
    this._validatePath(path, true);

    // Clobber logic
    let fileExistsAtPath = await this.exists((permanent ? 'permanent/' : 'cache/') + _fileName);
