...
export class FileSystem {
   ...
   */
  async getLocalFilePathFromUrl(url, permanent, headers, fileName) {

    let filePath = null;

    let _fileName = fileName || await this.getFileNameFromUrl(url, headers);
    ...
    if (exists[0]) {
      ...
      let result = await this.fetchFile(url, permanent, _fileName, true, headers); // Clobber must be true to allow concurrent CacheableImage components with same source url (ie: bullet point images).
