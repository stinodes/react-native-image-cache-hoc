...
export class FileSystem {
  ...
  constructor({ cachePruneTriggerLimit, fileDirName }: Options) {
    this.os = Platform.OS;
    this.cachePruneTriggerLimit = cachePruneTriggerLimit;
   ...
   * @param absolute {Boolean} - whether or not the passed in path is absolute or relative. Defaults to relative since base path differs across platforms.
   * @returns {boolean} - Whether or not the file path is valid.
   * @throws error on bad filepath.
   * @private
   */
  _validatePath(path: string, absolute: boolean = false) {
    ...
    if (
      resolvedPath.substr(0, this.baseFilePath.length) !== this.baseFilePath
    ) {
   ...
   * @param headers {Object} - An object containing the headers for the remote call.
   * @param fileName {String} - Filename string.
   * @param extension {String} - Extension string.
  ...
  async getFileNameFromUrl(
    url: string,
    {
      ...
      fileName,
      extension,
    }: $Diff<InternalAPIOpts, { permanent: ?boolean, clobber: ?boolean }>,
  ): Promise<string> {
    ...
    if (!_extension) {
      switch (urlExt) {
        ...
        case 'bmp':
          ...
          break;
        default:
          _extension = await this.getExtensionFromContentTypeHeader(url, {
            headers,
            extension,
          });
      }
    ...
    return sha1(url).toString() + '.' + _extension;
  }
   ...
   */
  async getExtensionFromContentTypeHeader(
    url: string,
    {
      headers,
    }: $Diff<
      InternalAPIOpts,
      { clobber: ?boolean, permanent: ?boolean, fileName: ?string },
    >,
  ): Promise<?string> {
    let extension = null;
   ...
   * @param headers {Object} - An object containing the headers for the remote call.
   */
  async getLocalFilePathFromUrl(
    url: string,
    { permanent, fileName, extension, headers = {} }: PublicAPIOpts,
  ): Promise<string> {
    let filePath = null;

    let _fileName = await this.getFileNameFromUrl(url, {
      headers,
      extension,
      fileName,
    });
    let permanentFileExists = this.exists('permanent/' + _fileName);
    let cacheFileExists = this.exists('cache/' + _fileName);

    let exists = await Promise.all([permanentFileExists, cacheFileExists]);

    if (exists[0]) {
      filePath = this.baseFilePath + 'permanent/' + _fileName;
    } else if (exists[1]) {
      filePath = this.baseFilePath + 'cache/' + _fileName;
    } else {
      let result = await this.fetchFile(url, {
        permanent,
        fileName: _fileName,
        extension,
        headers,
        clobber: true,
      }); // Clobber must be true to allow concurrent CacheableImage components with same source url (ie: bullet point images).
      filePath = result.path;
    }

    //$FlowFixMe weird promise behavior
    return filePath;
  }
   ...
   * @param headers {Object} - an object containing headers for the remote call.
  ...
  async fetchFile(
    url: string,
    { permanent, fileName, extension, headers, clobber }: InternalAPIOpts,
  ) {
    let _fileName = await this.getFileNameFromUrl(url, {
      headers,
      fileName,
    ...
    let path =
      this.baseFilePath + (permanent ? 'permanent' : 'cache') + '/' + _fileName;
    this._validatePath(path, true);

    // Clobber logic
    let fileExistsAtPath = await this.exists(
      (permanent ? 'permanent/' : 'cache/') + _fileName,
    );
  ...
  async pruneFile(
    url: string,
    { fileName, extension, headers }: PublicAPIOpts,
  ) {
    if (!(await this.exists('cache'))) {
      return;
    }
    ...
    let permanentFileExists = this.exists('permanent/' + _fileName);
    let cacheFileExists = this.exists('cache/' + _fileName);
    if (permanentFileExists) await this.unlink('permanent/' + _fileName);
    if (cacheFileExists) await this.unlink('cache/' + _fileName);
  }
 ...
 * @returns {FileSystem}
 */
export default function FileSystemFactory(options: Options): FileSystem {
