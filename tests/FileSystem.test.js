...
describe('lib/FileSystem', function() {
  ...
  it('#getFileNameFromUrl should create a sha1 filename from a PNG/JPG/GIF/BMP url.', async () => {
    ...
    let pngFilename = await fileSystem.getFileNameFromUrl(
    ...
    let gifFilename = await fileSystem.getFileNameFromUrl(
      'https://upload.wikimedia.org/wikipedia/commons/2/2c/Rotating_earth_%28large%29.gif',
    );

    gifFilename.should.equal('c048132247cd28c7879ab36d78a8f45194640006.gif');

    let jpgFilename = await fileSystem.getFileNameFromUrl(
    ...
    let bmpFilename = await fileSystem.getFileNameFromUrl(
  ...
  it('#getLocalFilePathFromUrl should use the passed fileName when defined.', () => {
    ...
    return fileSystem
      ...
      .then(() => {
        RNFetchBlob.config.mock.calls[0][0].path.should.equal(
  ...
  it('#getLocalFilePathFromUrl should use the passed extension when defined.', () => {
    ...
    return fileSystem
      ...
      .then(() => {
        RNFetchBlob.config.mock.calls[0][0].path.should.equal(
          '/base/file/path/react-native-image-cache-hoc/cache/cd7d2199cd8e088cdfd9c99fc6359666adc36289.' +
            extension,
        );
      });
  });
  ...
  it('#fetchFile should validate path.', () => {
    ...
    return fileSystem
      .fetchFile('https://google.com/arbitrary.jpg', {
        permanent: true,
